...
-- GROUP BY 절에을 통해 그룹화된 결과 값의 범위를 제한할 때 사용자
...
-- RDBMS에서는 테이블 설계시 무결성 원칙으로 인해 동일한 정보가 여러 군데 존재하면 안되기 때문에
-- 필연적으로 데이블을 관리 목적에 맞게 설계 함. 
SELECT * FROM dept;
...
SELECT EMPNO, ENAME, SAL, E.DEPTNO, DNAME, LOC
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO  -- 동등 조인, 이너 조인(두 데이블이 일치하는 데이터만 선택)
    AND SAL <= 2500
    AND EMPNO <= 9999
    ORDER BY EMPNO;
...
-- 외부 조인은 동등 조인 조건을 만족하지 못해 노락되는 행을 출력하기 위해 사용
...
-- 각 부서별 최대 급여와 동일한 급열를 받는 사원 정보를 출력
SELECT *
FROM EMP
...
-- EXISTS 연산자 : 서브쿼리의 결과 값이 하나 이쌍 존재 하면 조건식이 모두 true, 존재하지 핞으면 모두 false
SELECT *
FROM EMP
