...
-- SUBSTR / SUBSTRB
--  데이터베이스 시작위치가 0이 아님, 2번째 매개변수는 길이, 3번째 매개변수를 생략하면 끝까지
SELECT
    JOB,
    SUBSTR(JOB, 1, 2),
    SUBSTR(JOB, 3, 2),
...
SELECT
    LTRIM('ssssssssslsdkjlkdjfdkdjdj            ', 's')
FROM
    DUAL;
...
SELECT
    EXTRACT(YEAR FROM DATE '2023-09-15') AS 년도추출
FROM
    DUAL;
...
FROM
    EMP
WHERE
    MONTHS_BETWEEN(SYSDATE, HIREDATE) / 12 >= 38;
...
SELECT
    TO_CHAR(SYSDATE, 'YY/MM/DD HH:MI:SS') AS "현재 날짜 시간"
FROM
    DUAL;
...
-- 3번
-- NEXT_DAY(기준일자, 찾을 요일) : 기준 일자 나음에 오는 날짜를 구하는 함수
